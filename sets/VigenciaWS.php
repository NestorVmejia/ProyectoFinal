...
/*
 * marco.gonzalezn@imss.gob.mx
 * IMSS Zacatecas CDI. 2013
 * 
 */

/**
 * Clase para obtener información de vigencia de derechos para el asegurado y su
 * grupo familiar, a traves de una multiconexion a WebServices del IMSS
 *
 * @author marco.gonzalezn
 */
//Las lib requeridas estan en "PEAR" que debe estar en el "include_path"
...
class VigenciaWS
{
    ...
    private
        ...
        $comFdsDH = array(
            'Colonia', 'Direccion', 'Telefono', 'Consultorio',
            'DhDeleg', 'DhUMF', 'ClavePresupuestal', 'Turno'
        ),
    ...
    {
        $acceUniPms = array('nss' => $NSS, 'Cpid' => '');
        if ($this->requestWebServiceData("Acceder_Uni", $acceUniPms)) {
            $this->parseResponseData();
        } else {

            //Solución para procesar la versión antigua del WS
            require 'imss/VigenciaWS/VigenciaWS_v1.php';
            $oldWS = new VigenciaWS_v1();
            $res = $oldWS->verificaVigencia($NSS);
            $this->data = $res['data'];
            $this->stat = $res['stat'];
        }
        $finalData = $this->data;
    ...
    {
        if (
            !is_array($data) || !is_array($data[$this->currWSData["root"]])
